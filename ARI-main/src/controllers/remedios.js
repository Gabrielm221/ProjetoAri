...
async function criaRemedio(req, res) {
  ...
  try {
    ...
    return res.status(201).json(novoRemedio); // Retorna o remédio criado com o status 201
  } catch (error) {
    console.error('Erro ao criar remédio:', error);
    return res.status(500).json({ error: 'Erro ao criar remédio.' }); // Retorna o erro com o status 500
  }
}
